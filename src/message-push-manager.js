...
function buildIgnoredAbnormalRegexList() {
  return [
    ...
    /自动农场 \/ 当前轮已停止/i,
    /地块交互面板未加载，施肥管理器未找到/,
    /plant interaction manager not found/i,
  ];
}
...
function interpretImmediateAbnormal(match, text) {
  ...
  if (
    lowered.includes("plant interaction manager not found")
    || rawText.includes("目标化肥容器不足")
    || rawText.includes("地块交互面板未加载")
    || rawText.includes("施肥管理器未找到")
  ) {
    return {
      headline: "自动施肥失败",
      summary: "地块交互面板未加载，施肥管理器未找到。不是肥料库存不足。",
      fingerprintBase: "auto_fertilizer_interaction_manager_missing",
      fingerprintText: "auto_fertilizer_interaction_manager_missing",
    };
  }
...
function isNonPushAbnormalWarningText(value) {
  const rawText = String(value == null ? "" : value).trim();
  if (!rawText) return false;
  const lowered = rawText.toLowerCase();
  return lowered.includes("plant interaction manager not found")
    || rawText.includes("地块交互面板未加载")
    || rawText.includes("施肥管理器未找到");
}
...
class MessagePushManager {
  ...
  async _consumeObservedText(line, options) {
    ...
    const opts = options && typeof options === "object" ? options : {};
    if (!this.abnormalPushArmed) return;
    if (isNonPushAbnormalWarningText(text)) return;
