...
const WAREHOUSE_DEFAULT_REFRESH_INTERVAL_MIN = 5;
const WAREHOUSE_REFRESH_INTERVAL_MS = WAREHOUSE_DEFAULT_REFRESH_INTERVAL_MIN * 60 * 1000;
...
const GAME_CTL_READ_CACHE_TTL_MS = Object.freeze({
  ...
  "gameCtl.getOtherPlaceLoginReconnectState": 1_500,
});
...
const FARM_RUNTIME_EVENT_BINDING = "__qqFarmRuntimeEventBinding";
...
const FARM_CONFIG_DEFAULT = {
  ...
  autoFarmFriendVisitCooldownMin: 5,
  ...
  warehouseRefreshIntervalMin: WAREHOUSE_DEFAULT_REFRESH_INTERVAL_MIN,
  ...
  otherPlaceLoginReconnect: normalizeOtherPlaceLoginReconnectConfig({}),
};
...
function normalizeOtherPlaceLoginReconnectConfig(input) {
  const src = input && typeof input === "object" ? input : {};
  return {
    enabled: src.enabled === true,
    reconnectDelayMin: clampInt(src.reconnectDelayMin, 5, 0, 1440),
  };
}
...
async function loadFarmConfig() {
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      merged.autoFarmFriendVisitCooldownMin = clampInt(
        merged.autoFarmFriendVisitCooldownMin,
        5,
        0,
        1440
      );
      merged.autoFarmFriendBlacklistCooldownMin = clampInt(
        merged.autoFarmFriendBlacklistCooldownMin,
        10,
        0,
        1440
      );
      merged.autoFarmFertilizerInterLandIntervalSec = clampInt(
        merged.autoFarmFertilizerInterLandIntervalSec,
        0,
        0,
        0
      );
      merged.autoFarmFertilizerContinuousRushEnabled = merged.autoFarmFertilizerContinuousRushEnabled === true;
      ...
      merged.warehouseAutoRefreshEnabled = merged.warehouseAutoRefreshEnabled !== false && merged.warehouseRefreshOnlyOnAutoSell !== true;
      merged.warehouseRefreshIntervalMin = normalizeWarehouseRefreshIntervalMin(merged.warehouseRefreshIntervalMin);
      merged.warehouseRefreshOnlyOnAutoSell = merged.warehouseRefreshOnlyOnAutoSell === true;
      ...
      merged.otherPlaceLoginReconnect = normalizeOtherPlaceLoginReconnectConfig(parsed.otherPlaceLoginReconnect);
      return merged;
    }
  } catch (_) {
    /* 无文件或解析失败 */
  ...
  return {
    ...
    processGuard: normalizeProcessGuardConfig(FARM_CONFIG_DEFAULT.processGuard),
    otherPlaceLoginReconnect: normalizeOtherPlaceLoginReconnectConfig(FARM_CONFIG_DEFAULT.otherPlaceLoginReconnect),
  };
}
...
async function saveFarmConfig(partial) {
  ...
  next.autoFarmFriendVisitCooldownMin = clampInt(
    next.autoFarmFriendVisitCooldownMin,
    5,
    0,
    1440
  );
  next.autoFarmFriendBlacklistCooldownMin = clampInt(
    next.autoFarmFriendBlacklistCooldownMin,
    10,
    0,
    1440
  );
  next.autoFarmFertilizerInterLandIntervalSec = clampInt(
    next.autoFarmFertilizerInterLandIntervalSec,
    0,
    0,
    0
  );
  next.autoFarmFertilizerContinuousRushEnabled = next.autoFarmFertilizerContinuousRushEnabled === true;
  ...
  next.warehouseAutoRefreshEnabled = next.warehouseAutoRefreshEnabled !== false && next.warehouseRefreshOnlyOnAutoSell !== true;
  next.warehouseRefreshIntervalMin = normalizeWarehouseRefreshIntervalMin(next.warehouseRefreshIntervalMin);
  next.warehouseRefreshOnlyOnAutoSell = next.warehouseRefreshOnlyOnAutoSell === true;
  ...
  next.otherPlaceLoginReconnect = normalizeOtherPlaceLoginReconnectConfig(next.otherPlaceLoginReconnect);
...
function normalizeWarehouseRefreshIntervalMin(value) {
  const num = Number(value);
  if (!Number.isFinite(num)) return WAREHOUSE_DEFAULT_REFRESH_INTERVAL_MIN;
...
function enrichFriendItem(friend, config) {
  ...
  const blockedForAny = blockedForSteal || blockedForHelp;
  return {
    ...friend,
...
function summarizeFriendCounts(list) {
  ...
  friends.forEach((friend) => {
    ...
    if (friend && friend.flags && friend.flags.blockedForSteal) counts.blockedFriends += 1;
...
function normalizeWarehouseItemForUi(raw, index) {
  ...
  const mutation = Number(item.mutation || item.mutationNum || item.mutantNum || item.mutant_num) || 0;
  const category = classifyWarehouseItem(itemId, mergedItemInfo, item.type);
  const sourceTabNames = Array.isArray(item.sourceTabNames) ? item.sourceTabNames : [];
  const categoryDetail = category === "fruit" && sourceTabNames.some((name) => /超变/.test(String(name || "")))
    ? "mutant_fruit"
    : category;
...
async function fetchSeedAvailabilityForUi({ ensureAutomationSession, ensureAutomationGameCtl, callAutomationGameCtl, maxLevel }) {
  const normalizedMaxLevel = Number(maxLevel) || 0;
  const cached = getUiReadCache(`seedAvailability:${normalizedMaxLevel || "default"}`);
  if (cached) return cached;
  const session = await ensureAutomationSession();
  await ensureAutomationGameCtl(session);
  ...
  const shopList = await callAutomationGameCtl(session, "gameCtl.getShopSeedList", [{
    sortByLevel: true,
    silent: true,
    playerLevel: normalizedMaxLevel,
  }]);
  return setUiReadCache(`seedAvailability:${normalizedMaxLevel || "default"}`, UI_READ_CACHE_TTL_MS.seedAvailability, {
...
async function shovelLandForUi({ ensureAutomationSession, ensureAutomationGameCtl, callAutomationGameCtl, body }) {
  const session = await ensureAutomationSession();
  await ensureAutomationGameCtl(session);
  const input = body && typeof body === "object" ? body : {};
  const args = [{
    ...
    waitAfterOpen: input.waitAfterOpen,
    waitAfterAction: input.waitAfterAction,
    cleanupUi: input.cleanupUi !== false,
    captureProtocol: input.captureProtocol === true,
  }];
  if (input.landId != null) args[0].landId = Number(input.landId);
  if (input.path) args[0].path = String(input.path);
  const payload = await callAutomationGameCtl(session, "gameCtl.shovelLand", args);
  return payload && typeof payload === "object" ? payload : null;
}

async function shovelLandsBatchForUi({ ensureAutomationSession, ensureAutomationGameCtl, callAutomationGameCtl, body }) {
  const session = await ensureAutomationSession();
  await ensureAutomationGameCtl(session);
  const input = body && typeof body === "object" ? body : {};
  const landIds = normalizePositiveIntList(
    Array.isArray(input.landIds)
      ? input.landIds
      : (Array.isArray(input.landIdList) ? input.landIdList : (input.landId != null ? [input.landId] : []))
  );
  const args = [{
    silent: true,
    dryRun: input.dryRun !== false,
    landIds,
    waitAfterAction: input.waitAfterAction,
    betweenLandWait: input.betweenLandWait,
  }];
  const payload = await callAutomationGameCtl(session, "gameCtl.shovelLandsBatch", args, {
    timeoutMs: Math.max(30_000, Number(input.timeoutMs) || 90_000),
  });
  return payload && typeof payload === "object" ? payload : null;
}
...
async function performFriendOperation({ session, callAutomationGameCtl, target, action, enterWaitMs, actionWaitMs, returnHome }) {
  ...
  async function readFarmStatus(options = {}) {
    return await getExecutorFarmStatus(session, callAutomationGameCtl, {
      ...
      includeLandIds: options.includeLandIds === true,
      silent: true,
    });
  }
...
function createGateway(config) {
  ...
  let cdpRuntimeEventListenerAttached = false;

  function extractConsoleArgText(arg) {
    if (arg == null) return "";
  ...
  function parseOtherPlaceLoginRuntimeConsoleEvent(params) {
    const args = Array.isArray(params && params.args) ? params.args : [];
    if (!args.length) return null;
    const first = extractConsoleArgText(args[0]);
    const prefix = "[qq-farm-runtime-event]";
    if (!first || !first.startsWith(prefix)) return null;
    const raw = first.slice(prefix.length);
    try {
  ...
  function parseOtherPlaceLoginRuntimeBindingEvent(params) {
    if (!params || params.name !== FARM_RUNTIME_EVENT_BINDING) return null;
  ...
  async function installFarmRuntimeEventBridge(session) {
    if (!session || typeof session.sendCommand !== "function") return;
    try {
      await session.sendCommand("Runtime.addBinding", { name: FARM_RUNTIME_EVENT_BINDING }, 1500);
    } catch (_) {}
    try {
      await session.sendCommand("Page.addScriptToEvaluateOnNewDocument", {
        source: `globalThis.__qqFarmRuntimeEventBridge = function(payload) { try { globalThis.${FARM_RUNTIME_EVENT_BINDING}(JSON.stringify(payload || {})); } catch (_) {} };`,
      }, 1500);
    } catch (_) {}
    if (typeof session.evaluate === "function") {
      try {
        await session.evaluate(
          `(function(){ globalThis.__qqFarmRuntimeEventBridge = function(payload) { try { globalThis.${FARM_RUNTIME_EVENT_BINDING}(JSON.stringify(payload || {})); } catch (_) {} }; return true; })()`,
          { awaitPromise: false, timeoutMs: 1500 },
        );
      } catch (_) {}
    }
  }

  async function refreshFarmRuntimeEventBridge() {
    if (cdp) {
      await installFarmRuntimeEventBridge(cdp);
    }
  }

  function attachOtherPlaceLoginRuntimeEventListeners(session) {
    if (!session || cdpRuntimeEventListenerAttached) return;
    const onConsole = (params) => {
      const event = parseOtherPlaceLoginRuntimeConsoleEvent(params);
      if (event) handleOtherPlaceLoginRuntimeEvent(event, "console");
    };
    const onBinding = (params) => {
      const event = parseOtherPlaceLoginRuntimeBindingEvent(params);
      if (event) handleOtherPlaceLoginRuntimeEvent(event, "binding");
    };
    const onRuntimeEvent = (params) => {
      if (params && params.name === "other_place_login_reconnect") {
        handleOtherPlaceLoginRuntimeEvent(params, "event");
      }
    };
    session.on("Runtime.consoleAPICalled", onConsole);
    session.on("Runtime.bindingCalled", onBinding);
    session.on("event", onRuntimeEvent);
    session.on("cdpEvent", (message) => {
      if (message && message.method === "Runtime.consoleAPICalled") {
        const event = parseOtherPlaceLoginRuntimeConsoleEvent(message.params || {});
        if (event) handleOtherPlaceLoginRuntimeEvent(event, "cdpEvent");
      }
      if (message && message.method === "Runtime.bindingCalled") {
        const event = parseOtherPlaceLoginRuntimeBindingEvent(message.params || {});
        if (event) handleOtherPlaceLoginRuntimeEvent(event, "bindingEvent");
      }
    });
    cdpRuntimeEventListenerAttached = true;
  }
    ...
    if (!ensureCdpInFlight) {
      ensureCdpInFlight = (async () => {
        try {
          ...
          await cdp.connect();
          attachOtherPlaceLoginRuntimeEventListeners(cdp);
          await installFarmRuntimeEventBridge(cdp);
          return cdp;
        } catch (error) {
          if (cdp) {
  ...
  qqWsSession.on("hello", (_snapshot, client) => {
    ...
    if (hello.gameCtlReady === true) {
      ...
      syncOtherPlaceLoginReconnectConfigInBackground(currentConfig, "qq_ws_hello_ready");
    }
  ...
  qqWsSession.on("event", (entry) => {
    ...
    if (!payload) return;
    if (handleOtherPlaceLoginRuntimeEvent(payload, "qq_ws")) return;
    if (payload.name !== "gameCtlReadyChanged" || payload.ready !== true) return;
    if (processGuard) {
      processGuard.noteHealthy({ snapshot: getAutomationTransportState() });
    }
    const currentConfig = autoFarmManager.getState().config || FARM_CONFIG_DEFAULT;
    syncOtherPlaceLoginReconnectConfigInBackground(currentConfig, "qq_ws_gamectl_ready");
  ...
  async function ensureAutomationSession() {
    ...
    try {
      ...
      await refreshFarmRuntimeEventBridge();
      if (processGuard) processGuard.noteHealthy({ snapshot: getAutomationTransportState() });
      return session;
    } catch (error) {
      if (processGuard) {
  ...
  function buildOtherPlaceLoginReconnectRuntimeOptions(configData) {
    const settings = normalizeOtherPlaceLoginReconnectConfig(
      configData && configData.otherPlaceLoginReconnect,
    );
    return {
      settings,
      opts: {
        ...
        intervalMs: 5_000,
        waitAfter: 250,
        reconnectDelayMs: settings.reconnectDelayMin * 60 * 1000,
      },
    };
  }

  async function syncOtherPlaceLoginReconnectConfigToRuntime(configData, options = {}) {
    const { settings, opts } = buildOtherPlaceLoginReconnectRuntimeOptions(configData);
    const timeoutMs = Math.max(1000, Number(options.timeoutMs) || 4000);
    const resolvedTarget = resolveAutomationRuntimeTarget();
    const payload = {
      ok: false,
    ...
    try {
      if (resolvedTarget === "qq_ws") {
        ...
        const describe = await qqWsSession.call("host.describe", [], { timeoutMs });
        const methods = Array.isArray(describe && describe.availableMethods) ? describe.availableMethods : [];
        if (!methods.includes("gameCtl.setOtherPlaceLoginReconnectEnabled")) {
          payload.skipped = true;
          payload.reason = "runtime_missing_method";
          return payload;
        }
      ...
      const cdpSnapshot = getCdpSnapshot();
      if (!settings.enabled && !(cdpSnapshot && cdpSnapshot.contextReady)) {
        payload.skipped = true;
        payload.reason = "runtime_not_ready";
        return payload;
      }
      const session = await ensureAutomationSession();
      await ensureAutomationGameCtl(session);
      const state = await callAutomationGameCtl(
        session,
        "gameCtl.setOtherPlaceLoginReconnectEnabled",
        [settings.enabled, opts],
        { timeoutMs, noCache: true },
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      payload.error = err.message;
      payload.reason = /not ready|not connected|timeout|execution context|Target/i.test(err.message)
        ? "runtime_not_ready"
        : "runtime_sync_failed";
      return payload;
    }
  ...
  function syncOtherPlaceLoginReconnectConfigInBackground(configData, reason) {
    void syncOtherPlaceLoginReconnectConfigToRuntime(configData, { reason })
      .then((result) => {
        if (!result || result.ok || result.skipped) return;
        console.warn(`[gateway][other_place_login] sync failed: ${result.error || result.reason || "unknown"}`);
      })
      .catch((error) => {
        const err = error instanceof Error ? error : new Error(String(error));
        console.warn(`[gateway][other_place_login] sync failed: ${err.message}`);
      });
  }
  ...
  const abnormalTaskEventLogStore = new TaskEventLogStore(projectRoot, {
    filename: "task-abnormal-events.ndjson",
  });
  let wss = null;

  function formatReconnectDelayText(ms) {
    const value = Math.max(0, Number(ms) || 0);
    if (value >= 60 * 1000) {
      return `${Math.ceil(value / 60_000)}分钟`;
    }
    return `${Math.ceil(value / 1000)}秒`;
  }

  async function appendOtherPlaceLoginAbnormalLog(message, extra = {}) {
    const entry = normalizeTaskEventEntry({
      level: "warn",
      category: "other_place_login_reconnect",
      message,
      extra: {
        source: "other_place_login_reconnect",
        ...extra,
      },
    });
    await taskEventLogStore.append(entry);
    await abnormalTaskEventLogStore.append(entry);
    broadcastControlEvent("autoFarmEvent", {
      entry,
    });
  }

  const otherPlaceLoginRuntimeEventKeys = new Set();

  function rememberOtherPlaceLoginRuntimeEvent(key) {
    const text = String(key || "");
    if (!text) return false;
    if (otherPlaceLoginRuntimeEventKeys.has(text)) return false;
    otherPlaceLoginRuntimeEventKeys.add(text);
    if (otherPlaceLoginRuntimeEventKeys.size > 200) {
      const first = otherPlaceLoginRuntimeEventKeys.values().next().value;
      otherPlaceLoginRuntimeEventKeys.delete(first);
    }
    return true;
  }

  function handleOtherPlaceLoginRuntimeEvent(payload, source) {
    const event = payload && typeof payload === "object" ? payload : {};
    if (event.name !== "other_place_login_reconnect") return false;
    const phase = String(event.phase || "");
    const key = [
      phase,
      event.firstDetectedAt || "",
      event.handledAt || "",
      event.reconnectDelayMs || "",
      event.remainingMs || "",
      source || "",
    ].join("|");
    if (!rememberOtherPlaceLoginRuntimeEvent(key)) return true;
    if (phase === "waiting") {
      void appendOtherPlaceLoginAbnormalLog(
        `账户在别处登陆，等待${formatReconnectDelayText(event.reconnectDelayMs)}重连`,
        {
          phase,
          source,
          firstDetectedAt: event.firstDetectedAt || null,
          remainingMs: event.remainingMs != null ? event.remainingMs : null,
          reconnectDelayMs: event.reconnectDelayMs != null ? event.reconnectDelayMs : null,
    ...
    if (phase === "reconnected") {
      void appendOtherPlaceLoginAbnormalLog("账户在别处登陆，重连成功", {
        phase,
        source,
  ...
  function getTaskEventText(entry) {
    ...
    const event = extra && extra.event && typeof extra.event === "object" ? extra.event : null;
    if (event) {
      parts.push(event.message, event.module, event.action, event.farmType, event.reason, event.skipReason);
    }
    return parts.filter((item) => item != null).map((item) => String(item)).join(" ");
  }

  function isAbnormalTaskEvent(entry) {
    const level = String(entry && entry.level || "info").toLowerCase();
    if (level === "error" || level === "fatal") return true;
    const category = String(entry && entry.category || "").toLowerCase();
    if (category === "auto_fertilizer_interaction_manager_warning") return false;
    if (category === "other_place_login_reconnect") return true;
    if (/fail|failed|error|timeout|persist_failed/.test(category)) return true;
    const text = getTaskEventText(entry);
    if (/失败\s*0\s*块/.test(text)) return false;
    if (/地块交互面板未加载|施肥管理器未找到|plant interaction manager not found/i.test(text)) return false;
    return /调度异常|网络异常|连接异常|运行时异常|错误|超时|断开|未就绪|无法|error|failed|failure|timeout/i.test(text);
  }
  ...
  async function getRecentAbnormalTaskLogEntries(limit) {
    const resolvedLimit = Math.min(10000, Math.max(1, Number(limit) || 500));
    const persisted = await abnormalTaskEventLogStore.readRecent(resolvedLimit);
    const runtimeState = autoFarmManager.getState();
    const recentEvents = Array.isArray(runtimeState && runtimeState.recentEvents)
      ? runtimeState.recentEvents.filter(isAbnormalTaskEvent).slice(-resolvedLimit)
      : [];
    if (!recentEvents.length) {
      return persisted;
    }
  ...
  function getWarehouseConfig() {
    ...
    const configData = state && state.config && typeof state.config === "object"
      ...
      : FARM_CONFIG_DEFAULT;
    return {
      warehouseAutoRefreshEnabled: configData.warehouseAutoRefreshEnabled !== false && configData.warehouseRefreshOnlyOnAutoSell !== true,
      warehouseRefreshIntervalMin: normalizeWarehouseRefreshIntervalMin(configData.warehouseRefreshIntervalMin),
      warehouseRefreshOnlyOnAutoSell: configData.warehouseRefreshOnlyOnAutoSell === true,
  ...
  function getWarehouseRefreshIntervalMs(configData) {
    const cfg = configData || getWarehouseConfig();
    return normalizeWarehouseRefreshIntervalMin(cfg.warehouseRefreshIntervalMin) * 60 * 1000;
  }
  ...
  function getWarehouseNextAutoSellAt(configData) {
    ...
    const baseAt = Math.max(
      warehouseRuntimeState.lastSellAt || 0,
      warehouseRuntimeState.lastAutoSellCheckAt || 0,
      warehouseRuntimeState.bootAt || 0
    );
  ...
  function buildWarehouseSchedulerState() {
    ...
    const refreshIntervalMs = getWarehouseRefreshIntervalMs(cfg);
    return {
      refreshIntervalSec: Math.round(refreshIntervalMs / 1000),
      refreshIntervalMin: cfg.warehouseRefreshIntervalMin,
      autoRefreshEnabled: cfg.warehouseAutoRefreshEnabled === true,
      refreshOnlyOnAutoSell: cfg.warehouseRefreshOnlyOnAutoSell === true,
      ...
      lastAutoSellCheckAt: warehouseRuntimeState.lastAutoSellCheckAt || null,
  ...
  function getWarehouseCachedPayload() {
    ...
    return {
      ...
      capabilities: buildWarehouseCapabilities({
        ...(base.capabilities || {}),
        refreshIntervalSec: scheduler.refreshIntervalSec,
        refreshIntervalMin: scheduler.refreshIntervalMin,
        autoRefreshEnabled: scheduler.autoRefreshEnabled,
        refreshOnlyOnAutoSell: scheduler.refreshOnlyOnAutoSell,
  ...
  function scheduleWarehouseMaintenance(delayMs) {
    ...
    const targetDelay = delayMs != null
      ? Number(delayMs)
      : Math.max(1000, (Number(warehouseRuntimeState.nextRefreshAt) || now + refreshIntervalMs) - now);
    warehouseRuntimeState.timer = setTimeout(() => {
      warehouseRuntimeState.timer = null;
      void runWarehouseMaintenanceTick();
    }, Math.max(1000, Number.isFinite(targetDelay) ? targetDelay : refreshIntervalMs));
  }
  ...
  function updateWarehouseCacheFromItems(items, meta = {}) {
    ...
    warehouseRuntimeState.nextRefreshAt = cfg.warehouseRefreshOnlyOnAutoSell === true
      ? getWarehouseNextAutoSellAt(cfg)
      : now + getWarehouseRefreshIntervalMs(cfg);
  ...
  function getWarehouseNextMaintenanceAt(configData, now = Date.now()) {
    const cfg = configData || getWarehouseConfig();
    if (cfg.warehouseRefreshOnlyOnAutoSell === true) {
      if (cfg.autoWarehouseSellEnabled !== true) return null;
      return getWarehouseNextAutoSellAt(cfg) || now;
    }
    if (cfg.warehouseAutoRefreshEnabled !== true) return null;
    return warehouseRuntimeState.nextRefreshAt || (now + getWarehouseRefreshIntervalMs(cfg));
  }
  ...
  async function executeWarehouseRefreshOperation({ reason, allowAutoSell = true } = {}) {
    ...
    try {
      ...
      const autoSellDue = allowAutoSell && cfg.autoWarehouseSellEnabled === true && isWarehouseAutoSellDue(cfg);
      if (autoSellDue) {
        warehouseRuntimeState.lastAutoSellCheckAt = Date.now();
      }
      if (autoSellDue && candidateIds.length > 0) {
        try {
  ...
  async function runWarehouseMaintenanceTick() {
    const now = Date.now();
    const cfg = getWarehouseConfig();
    const nextMaintenanceAt = getWarehouseNextMaintenanceAt(cfg, now);
    if (nextMaintenanceAt == null) {
      warehouseRuntimeState.nextRefreshAt = null;
      scheduleWarehouseMaintenance(60 * 1000);
      return;
    }
    warehouseRuntimeState.nextRefreshAt = nextMaintenanceAt;
    ...
    try {
      ...
      if (isWarehouseAutoFarmBusyTimeoutError(err)) {
        ...
        warehouseRuntimeState.nextRefreshAt = cfg.warehouseRefreshOnlyOnAutoSell === true
          ? getWarehouseNextAutoSellAt(cfg)
          : Date.now() + getWarehouseRefreshIntervalMs(cfg);
      }
  ...
  loadFarmConfig()
    .then((savedConfig) => {
      ...
      syncOtherPlaceLoginReconnectConfigInBackground(savedConfig, "config_loaded");
    ...
    .catch(() => {
      ...
      syncOtherPlaceLoginReconnectConfigInBackground(FARM_CONFIG_DEFAULT, "config_load_failed");
      autoFarmBootConfigLoaded = true;
      scheduleWarehouseMaintenance(3000);
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && urlPath === "/api/farm-config") {
      try {
        ...
        syncOtherPlaceLoginReconnectConfigInBackground(data, "farm_config_saved");
    ...
    if (req.method === "GET" && urlPath === "/api/task-logs") {
      try {
        ...
        const type = String(parsedUrl.searchParams.get("type") || "").trim().toLowerCase();
        const data = type === "abnormal"
          ? await getRecentAbnormalTaskLogEntries(limit)
          : await getRecentTaskLogEntries(limit);
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify({
          ok: true,
          data: {
            limit,
            type: type === "abnormal" ? "abnormal" : "all",
            items: data,
          },
        }));
      } catch (e) {
    ...
    if (req.method === "POST" && urlPath === "/api/task-logs/clear") {
      try {
        await taskEventLogStore.clear();
        const state = autoFarmManager.clearRecentEvents();
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify({
          ok: true,
          data: {
      ...
      try {
        ...
        try {
          const fetched = await fetchSeedAvailabilityForUi({
            ...
            ensureAutomationGameCtl,
            callAutomationGameCtl,
            maxLevel: levelInfo.effectiveMaxLevel,
          });
          availability = {
            ...fetched,
    ...
    if (req.method === "POST" && urlPath === "/api/shovel-land") {
      try {
        const body = await readJsonBody(req);
        const data = await shovelLandForUi({
          ensureAutomationSession,
          ensureAutomationGameCtl,
          callAutomationGameCtl,
          body,
    ...
    if (req.method === "POST" && urlPath === "/api/shovel-lands-batch") {
      try {
        const body = await readJsonBody(req);
        const data = await shovelLandsBatchForUi({
          ensureAutomationSession,
          ensureAutomationGameCtl,
          callAutomationGameCtl,
          body,
    ...
    if (req.method === "POST" && urlPath === "/api/auto-farm") {
      try {
        ...
        if (parsed.config && typeof parsed.config === "object") {
          ...
          messagePushManager.updateConfig(savedConfig.messagePush);
          processGuard.updateConfig(savedConfig.processGuard);
          syncOtherPlaceLoginReconnectConfigInBackground(savedConfig, "auto_farm_config_saved");
          scheduleWarehouseMaintenance(1500);
        }
