...
function collectWorkLandIdSet(status, key) {
  ...
  const set = new Set();
  for (let i = 0; i < source.length; i += 1) {
    const landId = Number(source[i]);
    if (Number.isFinite(landId) && landId > 0) set.add(landId);
  }
  return set;
}
...
function collectAllowedStealTargets(status, stealPlantBlacklist) {
  ...
  const collectLandIdSet = collectWorkLandIdSet(status, "collect");
  ...
  for (let i = 0; i < grids.length; i += 1) {
    ...
    const targetInCollectSet = targetLandId > 0 && collectLandIdSet.has(targetLandId);
    // Friend-farm one-click harvest can be sourced from manager land IDs even when grid.canSteal is false.
    const canSteal = grid.canSteal === true || grid.canCollect === true || collectLandIdSet.has(landId) || targetInCollectSet;
    const multiTileFallbackCollectable = !!(
      multiTileHarvest
      && multiTileAnchorGrid
      && (multiTileAnchorGrid.canSteal === true || multiTileAnchorGrid.canCollect === true || targetInCollectSet)
      && grid.hasPlant
      && !grid.isDead
...
function resolveAutoFertilizerInterLandWaitMs(opts) {
  const explicitMs = Number(opts && opts.fertilizeInterLandWaitMs);
  if (Number.isFinite(explicitMs) && explicitMs >= 0) {
    return Math.floor(explicitMs);
  }
  const seconds = Number(opts && opts.autoFertilizerInterLandIntervalSec);
...
function shouldRunAutoFertilizerHarvestLink(opts) {
  return !!(shouldRunAutoFertilizer(opts) && opts && (
    opts.autoFertilizerHarvestLinkEnabled === true
    || opts.autoFertilizerContinuousRushEnabled === true
  ));
}

function shouldRunAutoFertilizerContinuousRush(opts) {
  return !!(shouldRunAutoFertilizer(opts) && opts && opts.autoFertilizerContinuousRushEnabled === true);
}
...
function collectPlantedLandIdsFromAutoPlantResult(result) {
  const out = new Set();

  function addFromPlantAttempt(attempt) {
    if (!attempt || typeof attempt !== "object" || attempt.ok !== true) return;
    const requestedLandIds = normalizePositiveIntList(attempt.requestedLandIds);
    const beforeEmptySet = new Set(normalizePositiveIntList(attempt.beforeEmptyIds));
    const afterEmptySet = new Set(normalizePositiveIntList(attempt.afterEmptyIds));
    requestedLandIds.forEach((landId) => {
      if (beforeEmptySet.has(landId) && !afterEmptySet.has(landId)) {
        out.add(landId);
      }
    });
  }
  ...
  function visit(item) {
    ...
    addFromPlantAttempt(item);
    if (item.plantResult && typeof item.plantResult === "object") {
      addFromPlantAttempt(item.plantResult);
      visit(item.plantResult);
    }
    const attempts = Array.isArray(item.attempts) ? item.attempts : [];
    attempts.forEach(visit);
    const backpackAttempts = Array.isArray(item.backpackPlanAttempts) ? item.backpackPlanAttempts : [];
    backpackAttempts.forEach((entry) => {
      if (entry && entry.result) visit(entry.result);
    });
  }
...
function resetAutoFertilizerMarksForLandIds(opts, landIds) {
  const ids = normalizePositiveIntList(landIds);
  if (ids.length === 0) return 0;
  const state = getAutoFertilizerStateStore(opts);
  const marks = state.landMarks && typeof state.landMarks === "object"
    ? state.landMarks
    : (state.landMarks = Object.create(null));
  let removed = 0;
  ids.forEach((landId) => {
...
function buildAutoFertilizerSkippedAction(grid, fertilizerType, reason, extra) {
  ...
  return {
    ...
    autoFertilizerMode: grid && grid.autoFertilizerMode ? String(grid.autoFertilizerMode) : null,
...
function collectAutoFertilizerPlan(status, opts, phaseName) {
  ...
  const rushThresholdSec = getAutoFertilizerRushThresholdSec(opts);
  const continuousRushEnabled = shouldRunAutoFertilizerContinuousRush(opts);
  const state = getAutoFertilizerStateStore(opts);
  ...
  for (let i = 0; i < grids.length; i += 1) {
    const grid = grids[i];
    const gridWithMode = grid && typeof grid === "object"
      ? { ...grid, autoFertilizerMode: mode }
      : grid;
    if (!canUseAutoFertilizerOnGrid(grid, opts)) continue;
    const mark = getAutoFertilizerMarkForGrid(state, grid);
    if (!mark) continue;
    ...
    if (phaseName === "season_start") {
      if ((mode === "both" || mode === "both_legacy") && seasonStartDecision && seasonStartDecision.applyNow && mark.normalApplied === true) {
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "normal", "same_fertilizer_type_already_used", {
          phase: phaseName,
        ...
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "normal", "no_observed_effect_repeat", {
          phase: phaseName,
          fertilizerReason: "season_start_normal",
        }));
        ...
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "normal", "waiting_smart_phase", {
          phase: phaseName,
          fertilizerReason: "season_start_normal",
      ...
      if (mark.normalApplied === true && !continuousRushEnabled) {
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "normal", "same_fertilizer_type_already_used", {
          phase: phaseName,
      ...
      if (mark.organicApplied === true && !continuousRushEnabled) {
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "organic", "same_fertilizer_type_already_used", {
          phase: phaseName,
          fertilizerReason: "rush_organic",
        }));
      } else if (mark.organicBlocked === true) {
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "organic", "no_observed_effect_repeat", {
          phase: phaseName,
          fertilizerReason: "rush_organic",
        }));
      } else {
      ...
      && (mark.organicApplied !== true || continuousRushEnabled)
      && rushReady
    ) {
      if (mark.organicApplied === true && !continuousRushEnabled) {
        skippedActions.push(buildAutoFertilizerSkippedAction(gridWithMode, "organic", "same_fertilizer_type_already_used", {
...
async function runOwnFarmFertilizerPhase(session, callGameCtl, opts, phaseName) {
  ...
  const configuredInterChunkWaitMs = Number(opts && opts.fertilizeInterChunkWaitMs);
  const fertilizeInterChunkWaitMs = Math.max(0, Number.isFinite(configuredInterChunkWaitMs)
    ? configuredInterChunkWaitMs
    : 0);
  const fertilizeInterLandWaitMs = resolveAutoFertilizerInterLandWaitMs(opts);
  const fertilizeWaitAfterOpen = Math.max(100, Number(opts && opts.fertilizeWaitAfterOpen) || 350);
  const fertilizeWaitAfterAction = Math.max(100, Number(opts && opts.fertilizeWaitAfterAction) || 500);
  const fertilizerBatchCallTimeoutMs = Math.max(15_000, Number(opts && opts.fertilizeBatchCallTimeoutMs) || 30_000);
  const state = getAutoFertilizerStateStore(opts);
  throwIfAutomationStopped(opts);
  ...
  const needsBatchAttempt = candidateGroups.some((group) => Array.isArray(group) && group.length > 0);
  ...
  for (let groupIndex = 0; groupIndex < candidateGroups.length; groupIndex += 1) {
    ...
    for (let chunkIndex = 0; chunkIndex < groupChunks.length; chunkIndex += 1) {
      ...
      if (batchSupported === false) {
        const reason = batchUnavailableReason || "runtime_missing_gameCtl.fertilizeLandsBatch";
        for (let i = 0; i < runnableCandidates.length; i += 1) {
          const handled = applyAutoFertilizerErrorResult(state, runnableCandidates[i], fertilizerType, phase, reason);
      ...
      dispatchStats.batchCallCount += 1;
      reportProgress(opts, `自动施肥：批量直发 ${landIds.length} 块，类型=${fertilizerType}`, {
        module: "auto_fertilizer_batch_dispatch",
        phase,
        fertilizerType,
        landIds,
      ...
      try {
        batchResult = await fertilizeLandsBatch(session, callGameCtl, {
          ...
          switchWaitAfterOpen: 100,
          betweenLandWait: fertilizeInterLandWaitMs,
          retryResetWait: 0,
      ...
      if (batchResult && Array.isArray(batchResult.results) && batchResult.results.length > 0) {
        ...
        for (let i = 0; i < runnableCandidates.length; i += 1) {
          ...
          if (!entry) {
            dispatchStats.missingResultCount += 1;
            const handled = applyAutoFertilizerErrorResult(state, grid, fertilizerType, phase, "fertilize_batch_missing_result");
            actions.push(handled.action);
            continue;
          }
        ...
        dispatchStats.batchFailureCount += runnableCandidates.length;
        for (let i = 0; i < runnableCandidates.length; i += 1) {
          const grid = runnableCandidates[i];
        ...
        for (let i = 0; i < runnableCandidates.length; i += 1) {
          const grid = runnableCandidates[i];
          const handled = applyAutoFertilizerErrorResult(state, grid, fertilizerType, phase, "fertilize_batch_empty_result");
          actions.push(handled.action);
        }
      }
      ...
      if ((!isLastChunkInGroup || !isLastGroup) && fertilizeInterChunkWaitMs > 0) {
        await waitWithAutomationControl(fertilizeInterChunkWaitMs, opts);
      }
    }
  ...
  return {
    phase,
    autoFertilizerMode: normalizeAutoFertilizerMode(opts && opts.autoFertilizerMode),
    ok: failureCount === 0,
...
async function runOwnFarmFertilizerTasks(session, callGameCtl, opts) {
  ...
  async function runAutoFertilizerRushThenHarvest(phases) {
    ...
    let ok = true;
    const continuousRushEnabled = shouldRunAutoFertilizerContinuousRush(opts);
    const maxRounds = continuousRushEnabled
      ? Math.max(1, Math.min(500, Math.ceil(Math.max(30, rushThresholdSec) / 3)))
      : 1;
    ...
    for (let roundIndex = 0; roundIndex < maxRounds; roundIndex += 1) {
      ...
      const statusChanged = watchAfter.signature !== watchBefore.signature;
      const actionProgressMade = roundPhaseSuccessCount > 0 || roundCollectCount > 0;
      const progressMade = actionProgressMade || statusChanged;
      ...
      if ((opts && opts.stopOnError) && (harvestFailed || specialCollectFailed || roundPhaseFailureCount > 0)) {
        ok = false;
        stopReason = "stop_on_error";
        break;
      }
      if (watchAfter.activeCount <= 0) {
        if (roundPhaseSuccessCount > 0 && roundCollectCount > 0) {
          stopReason = "rush_fertilized_and_harvested";
        } else if (roundCollectCount > 0) {
          stopReason = "harvest_collected";
        } else if (roundPhaseSuccessCount > 0) {
          stopReason = "rush_fertilized_and_cleared";
        } else {
          stopReason = "watch_cleared";
        }
        break;
      }
      if (!continuousRushEnabled) {
        if (roundPhaseSuccessCount > 0) {
          stopReason = "rush_fertilized";
        } else if (roundCollectCount > 0) {
          stopReason = "harvest_collected";
        } else {
      ...
      if (!actionProgressMade || (roundPhaseSuccessCount > 0 && roundCollectCount <= 0 && !statusChanged)) {
        ok = false;
        stopReason = "no_progress";
        break;
      }

      stopReason = roundPhaseSuccessCount > 0 ? "rush_fertilized_continue" : "harvest_collected_continue";
    }

    if (continuousRushEnabled && rounds.length >= maxRounds && stopReason.endsWith("_continue")) {
      ok = false;
  ...
  if (!abortedReason && !runLimitReached) {
    if (shouldRunAutoFertilizerHarvestLink(opts) && rushPhases.length > 0) {
      linkedHarvest = await runAutoFertilizerRushThenHarvest(rushPhases);
  ...
  const dispatchStats = phaseResults.reduce((acc, item) => {
    ...
    acc.batchCallCount += Number(src.batchCallCount) || 0;
    acc.batchFailureCount += Number(src.batchFailureCount) || 0;
    acc.missingResultCount += Number(src.missingResultCount) || 0;
    return acc;
  }, {
    batchAttemptedCount: 0,
    batchCallCount: 0,
...
function collectMatureLandIds(status) {
  ...
  for (let i = 0; i < grids.length; i += 1) {
    ...
    const matureInSec = Number(targetGrid && targetGrid.matureInSec);
    const matchesSupplementalStage = stageKind === "mature"
      || grid.isMature === true
      || (Number.isFinite(matureInSec) && matureInSec <= 0);
...
async function getAvailableSeedCandidates(session, callGameCtl, opts) {
  ...
  try {
    ...
    shopList = await callGameCtl(session, "gameCtl.getShopSeedList", [withSilent({
      sortByLevel: true,
      playerLevel: effectiveLevel.maxLevel,
    })]);
  } catch (error) {
    shopList = [];
...
async function resolvePlantStrategy(session, callGameCtl, opts) {
  ...
  const {
    ...
    shopBySeedId,
    effectiveLevel,
  } = await getAvailableSeedCandidates(session, callGameCtl, opts);
  ...
  if (fertilizerEnabled) {
    try {
      ...
      if (fertilizerActive && plantedThisRound) {
        // Freshly replanted same-seed lands reuse the same land/plant season tuple.
        // Clear only the lands actually planted so the new crop can receive fertilizer.
        const plantedLandIds = collectPlantedLandIdsFromAutoPlantResult(plantResult);
        resetAutoFertilizerMarksForLandIds(opts, plantedLandIds);
...
async function runFriendStealAutomation(session, callGameCtl, opts) {
  ...
  const friendVisitCooldownMin = Math.max(0, Number(opts && opts.friendVisitCooldownMin) || 0);
  const friendBlacklistCooldownMin = Math.max(0, Number(opts && opts.friendBlacklistCooldownMin) || 0);
  ...
  const stealBlacklistScopeEnabled = shouldApplyFriendRuleScope(opts, "steal", "blacklist");
  const helpBlacklistScopeEnabled = shouldApplyFriendRuleScope(opts, "help", "blacklist");
  const stealMaskedBlacklistEnabled = stealBlacklistScopeEnabled && maskedBlacklistEnabled;
  const helpMaskedBlacklistEnabled = helpBlacklistScopeEnabled && maskedBlacklistEnabled;
  ...
  const stealMaskedBlockedFriends = friendList.filter((item) => (
    stealMaskedBlacklistEnabled
    && isFriendBelowLevel(item, maskedBlacklistMaxLevel)
  ));
  const stealBlacklistedFriends = friendList.filter((item) => (
    stealBlacklistScopeEnabled
    && isFriendBlacklisted(item, friendBlacklist)
  ));
  const helpMaskedBlockedFriends = friendList.filter((item) => (
    helpMaskedBlacklistEnabled
    && isFriendBelowLevel(item, maskedBlacklistMaxLevel)
  ));
  const helpBlacklistedFriends = friendList.filter((item) => (
    helpBlacklistScopeEnabled
    && isFriendBlacklisted(item, friendBlacklist)
  ));
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    try {
      ...
      const beforeStatus = await getFarmStatus(session, callGameCtl, {
        ...
        includeLandIds: stealEnabled || stealPlantBlacklist.length > 0,
      });
  ...
  return {
    ...
    helpLimitReached,
    helpState: normalizeFriendHelpState(helpState, null, helpDailyLimit),
    blacklistedCount: stealBlacklistedFriends.length + stealMaskedBlockedFriends.length,
    explicitBlacklistedCount: stealBlacklistedFriends.length,
    maskedBlockedCount: stealMaskedBlockedFriends.length,
    helpBlacklistedCount: helpBlacklistedFriends.length + helpMaskedBlockedFriends.length,
    helpExplicitBlacklistedCount: helpBlacklistedFriends.length,
    helpMaskedBlockedCount: helpMaskedBlockedFriends.length,
    cooldownBlockedCount: cooldownBlockedFriends.length,
    cooldownPolicy: {
      visitCooldownMin: friendVisitCooldownMin,
      blacklistCooldownMin: friendBlacklistCooldownMin,
    },
    maskedBlockedEnabled: stealMaskedBlacklistEnabled,
    helpMaskedBlockedEnabled: helpMaskedBlacklistEnabled,
    stealPlantBlacklistEnabled,
    stealPlantBlacklistStrategy,
    stealPlantBlacklist,
    blacklistPolicy,
    blacklistedFriends: stealBlacklistedFriends.map((friend) => ({
    ...
    maskedBlockedFriends: stealMaskedBlockedFriends.map((friend) => ({
      gid: friend && friend.gid != null ? friend.gid : null,
      displayName: friend && (friend.displayName || friend.name || friend.remark) ? (friend.displayName || friend.name || friend.remark) : null,
...
async function runAutoFarmCycle({ session, callGameCtl, options }) {
  ...
  if (ownFarmEnabled) {
    payload.ownFarm = await runOwnFarmAutomation(session, callGameCtl, {
      ...
      autoFertilizerContinuousRushEnabled: opts.autoFertilizerContinuousRushEnabled === true,
      ...
      autoFertilizerInterLandIntervalSec: opts.autoFertilizerInterLandIntervalSec,
