...
function buildAutoFertilizerSeasonKey(landId, plantId, currentSeason) {
  if (landId <= 0 || plantId <= 0 || currentSeason <= 0) return null;
...
function normalizeAutoFertilizerState(raw) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    ...
    const seasonKey = buildAutoFertilizerSeasonKey(landId, plantId, currentSeason);
...
function collectPlantedLandIdsFromAutoPlantResult(result) {
  ...
  function addFromPlantAttempt(attempt) {
    ...
    const beforeEmptySet = new Set(normalizeAutoFarmIdArray(attempt.beforeEmptyIds));
    const afterEmptySet = new Set(normalizeAutoFarmIdArray(attempt.afterEmptyIds));
...
function normalizeWarehouseRefreshIntervalMin(value) {
  const n = Number.parseInt(String(value ?? ""), 10);
  const fallback = Number.isFinite(n) ? n : WAREHOUSE_DEFAULT_REFRESH_INTERVAL_MIN;
  return Math.min(24 * 60, Math.max(1, fallback));
}
...
function normalizeAutoFarmConfig(raw) {
  ...
  return {
    ...
    autoFarmFriendVisitCooldownMin: toInt(src.autoFarmFriendVisitCooldownMin, 5, 0, 1440),
    autoFarmFriendBlacklistCooldownMin: toInt(src.autoFarmFriendBlacklistCooldownMin, 10, 0, 1440),
    ...
    autoFarmFertilizerContinuousRushEnabled: toBool(src.autoFarmFertilizerContinuousRushEnabled, false),
    ...
    warehouseAutoRefreshEnabled: toBool(src.warehouseAutoRefreshEnabled, true) && !toBool(src.warehouseRefreshOnlyOnAutoSell, false),
    warehouseRefreshIntervalMin: normalizeWarehouseRefreshIntervalMin(src.warehouseRefreshIntervalMin),
    warehouseRefreshOnlyOnAutoSell: toBool(src.warehouseRefreshOnlyOnAutoSell, false),
...
function resolveFriendCooldownMs(config, key, fallbackMs) {
  const minutes = Number(config && config[key]);
  if (!Number.isFinite(minutes)) return fallbackMs;
  return Math.max(0, Math.floor(minutes)) * 60 * 1000;
}

function buildFriendCooldown(config, key, fallbackMs, reason) {
  const ms = resolveFriendCooldownMs(config, key, fallbackMs);
  return ms > 0 ? { ms, reason } : null;
}

function shouldApplyFriendCooldown(visit, config) {
  if (!visit || typeof visit !== "object") return null;
  if (visit.reason === "no_collectable_after_enter") {
    return buildFriendCooldown(config, "autoFarmFriendVisitCooldownMin", FRIEND_VISIT_COOLDOWN_MS, "no_collectable_after_enter");
  }
  if (visit.reason === "no_actionable_after_enter") {
    return buildFriendCooldown(config, "autoFarmFriendVisitCooldownMin", FRIEND_VISIT_COOLDOWN_MS, "no_actionable_after_enter");
  }
  if (visit.reason === "all_collectable_blacklisted") {
    return buildFriendCooldown(config, "autoFarmFriendBlacklistCooldownMin", FRIEND_BLACKLIST_ONLY_COOLDOWN_MS, "all_collectable_blacklisted");
  }
  if (visit.reason === "blacklist_strategy_skip_whole_farm") {
    return buildFriendCooldown(config, "autoFarmFriendBlacklistCooldownMin", FRIEND_BLACKLIST_ONLY_COOLDOWN_MS, "blacklist_strategy_skip_whole_farm");
  }
  const helpProgress = getHelpProgressFromVisit(visit);
  ...
  if (
    ...
    return buildFriendCooldown(config, "autoFarmFriendVisitCooldownMin", FRIEND_VISIT_COOLDOWN_MS, "no_progress_after_visit");
  }
  if (visit.ok === true && Number(visit.collectBefore) <= 0 && totalHelpProgress <= 0) {
    ...
    if ((helpBefore.water + helpBefore.eraseGrass + helpBefore.killBug) > 0) {
      return buildFriendCooldown(config, "autoFarmFriendVisitCooldownMin", FRIEND_VISIT_COOLDOWN_MS, "no_help_progress_after_visit");
    }
  }
  ...
  if (visit.ok === false && errorText) {
    if (
      ...
      return buildFriendCooldown(config, "autoFarmFriendVisitCooldownMin", FRIEND_VISIT_COOLDOWN_MS, "collect_race_or_stale_state");
    }
  }
  return null;
}
...
function formatAutoFarmFertilizerPhaseLabel(actionOrPhase) {
  ...
  const mode = String(action && action.autoFertilizerMode || "").trim().toLowerCase();
  ...
  if (text === "season_start") {
    if (mode === "both_legacy") {
      return followUpSeason ? "第二季种植无机轮" : "种植无机轮";
    }
...
function getAutoFarmFertilizerPhaseSummaryLabel(phaseOrItem) {
  const action = phaseOrItem && typeof phaseOrItem === "object" ? phaseOrItem : null;
  const text = String(action ? action.phase : phaseOrItem || "").trim().toLowerCase();
  const mode = String(action && action.autoFertilizerMode || "").trim().toLowerCase();
  if (text === "season_start" && mode === "both_legacy") return "种植无机";
...
function formatAutoFarmFertilizerReasonText(value) {
  const text = String(value || "").trim();
  if (!text) return "unknown";
  if (text.toLowerCase() === "plant interaction manager not found") return "地块交互面板未加载，施肥管理器未找到";
...
function formatAutoFarmFertilizerReason(action) {
  ...
  return formatAutoFarmFertilizerReasonText(action.reason || action.error);
}

function isAutoFarmFertilizerInteractionManagerWarning(action) {
  const text = String(action && (action.error || action.reason || action.displayReason) || "").trim().toLowerCase();
  return text === "plant interaction manager not found"
    || text.includes("地块交互面板未加载")
    || text.includes("施肥管理器未找到");
}
...
function buildAutoFarmCycleLogMessages({ due, injectState, result, cooldownApplied }) {
    ...
    if (fertilizerResult) {
      ...
      if (fertilizerResult.skipped) {
        ...
        phaseResults.forEach((item) => {
          ...
          summaryParts.push(`${getAutoFarmFertilizerPhaseSummaryLabel(item)} ${candidateCount} 块`);
        });
        messages.push({
          level: "info",
      ...
      if (!fertilizerResult.skipped && fertilizerResult.ok) {
        ...
        messages.push({
          level: "warn",
          message: `自动农场 / 自己农场 / 自动施肥：失败 ${formatAutoFarmFertilizerReasonText(fertilizerResult.error || fertilizerResult.reason)} · 成功 ${Number(fertilizerResult.successCount) || 0} 块，跳过 ${Number(fertilizerResult.skippedCount) || 0} 块，失败 ${Number(fertilizerResult.failureCount) || 0} 块`,
        });
      }
      ...
      if (fertilizerDispatchStats) {
        ...
        const batchCallCount = Number(fertilizerDispatchStats.batchCallCount) || 0;
        const batchFailureCount = Number(fertilizerDispatchStats.batchFailureCount) || 0;
        const missingResultCount = Number(fertilizerDispatchStats.missingResultCount) || 0;
        ...
        if (
          ...
          const statParts = [
            `批量主跑 ${batchAttemptedCount} 块`,
          ];
          if (batchCallCount > 0 || batchFailureCount > 0 || missingResultCount > 0) {
            statParts.push(`批量调用 ${batchCallCount} 次`);
            statParts.push(`批量失败 ${batchFailureCount} 块`);
            statParts.push(`缺结果 ${missingResultCount} 块`);
          }
          if (fallbackBatchCount > 0 || fallbackSingleCount > 0 || duplicateFallbackBlockedCount > 0) {
            statParts.push(`批量补跑 ${fallbackBatchCount} 块`);
            statParts.push(`单块补跑 ${fallbackSingleCount} 块`);
            statParts.push(`拦截重复补跑 ${duplicateFallbackBlockedCount} 块`);
          }
          messages.push({
            level: "info",
            message: `自动农场 / 自己农场 / 自动施肥调度：${statParts.join(" · ")}`,
          });
        }
      }
      ...
      fertilizerActions.forEach((action) => {
        ...
        messages.push({
          level: "warn",
          message: `自动农场 / 自己农场 / 自动施肥：${parts.join(" · ")}`,
          category: isAutoFarmFertilizerInteractionManagerWarning(action)
            ? "auto_fertilizer_interaction_manager_warning"
            : null,
        });
      });
      const linkedHarvest = fertilizerResult.linkedHarvest && typeof fertilizerResult.linkedHarvest === "object"
  ...
  if (friendPatrolEnabled && friendSteal) {
    if (friendSteal.skipped && friendSteal.skipReason === "quiet_hours") {
      ...
      const reportHelpOnly = friendHelpEnabled && !friendStealEnabled;
      const reportMixed = friendStealEnabled && friendHelpEnabled;
      const explicitBlacklistCount = reportHelpOnly
        ? Number(friendSteal.helpExplicitBlacklistedCount) || 0
        : Number(friendSteal.explicitBlacklistedCount) || 0;
      const maskedBlacklistCount = reportHelpOnly
        ? Number(friendSteal.helpMaskedBlockedCount) || 0
        : Number(friendSteal.maskedBlockedCount) || 0;
      if (explicitBlacklistCount > 0) {
        parts.push((reportHelpOnly ? "帮忙黑名单 " : "手动黑名单 ") + explicitBlacklistCount);
      }
      if (maskedBlacklistCount > 0) {
        parts.push((reportHelpOnly ? "帮忙等级屏蔽 " : "等级规则屏蔽 ") + maskedBlacklistCount);
      }
      if (reportMixed) {
        const helpExplicitBlacklistCount = Number(friendSteal.helpExplicitBlacklistedCount) || 0;
        const helpMaskedBlockedCount = Number(friendSteal.helpMaskedBlockedCount) || 0;
        if (helpExplicitBlacklistCount > 0) parts.push("帮忙黑名单 " + helpExplicitBlacklistCount);
        if (helpMaskedBlockedCount > 0) parts.push("帮忙等级屏蔽 " + helpMaskedBlockedCount);
      }
...
class AutoFarmManager {
  ...
  resetAutoFertilizerMarksForLandIds(landIds, opts) {
    const ids = normalizeAutoFarmIdArray(landIds);
    if (ids.length === 0) return { removedCount: 0, landIds: [] };
    ...
    if (removedCount > 0) {
      this.autoFertilizerState.updatedAt = new Date().toISOString();
      if (!(opts && opts.persist === false)) {
        void this._persistAutoFertilizerState(opts && opts.reason ? opts.reason : "reset_planted_lands");
      }
    }
    return { removedCount, landIds: ids };
  }
  ...
  replaceFriendHelpState(raw, opts) {
    ...
    if (options.markPersisted !== false) {
  ...
  _applyFriendVisitCooldowns(friendSteal, nowMs) {
    ...
    for (let i = 0; i < visits.length; i += 1) {
      ...
      const cooldown = shouldApplyFriendCooldown(visit, this.config);
  ...
  _buildTaskCycleOptions(taskId, runContext) {
    const baseOptions = {
      ...
      autoFertilizerContinuousRushEnabled: !!this.config.autoFarmFertilizerContinuousRushEnabled,
      ...
      autoFarmFertilizerMaxLandsPerRun: this.config.autoFarmFertilizerMaxLandsPerRun != null
        ...
        : 0,
      autoFertilizerInterLandIntervalSec: 0,
      ...
      friendVisitCooldownMin: this.config.autoFarmFriendVisitCooldownMin,
      friendBlacklistCooldownMin: this.config.autoFarmFriendBlacklistCooldownMin,
  ...
  async _runCycle(force, taskId) {
    ...
    try {
      ...
      if (result && result.friendSteal && result.friendSteal.helpEnabled === true) {
        this.replaceFriendHelpState(result.friendSteal.helpState, { persist: false, markPersisted: false });
      }
      if (selectedTaskId === "own_plant") {
        const plantResult = result && result.ownFarm && result.ownFarm.plantResult
          ? result.ownFarm.plantResult
          : null;
        if (plantResult && plantResult.ok === true && plantResult.action === "planted") {
          const plantedLandIds = collectPlantedLandIdsFromAutoPlantResult(plantResult);
          const resetResult = this.resetAutoFertilizerMarksForLandIds(plantedLandIds, { persist: false });
          if (resetResult.removedCount > 0) {
            pushCycleEvent("info", `自动农场 / 自动种植：已重置新种地块施肥状态 ${formatAutoFarmIdList(resetResult.landIds, 12)}`, {
      ...
      buildAutoFarmCycleLogMessages({
        ...
        pushCycleEvent(item.level || "info", item.message, {
          category: item.category || "cycle_detail",
        });
      });
